...
class LlmCallStatsContractTests(unittest.TestCase):
    ...
    def test_routes_are_read_only_and_unauthenticated_like_log_queries(self):
        ...
        self.assertIn("scenario_name: str = None", main)
        self.assertIn("run_id: str = None", main)
        self.assertIn("error_type: str = None", main)
        self.assertIn("keyword: str = None", main)
        self.assertIn("store.llm_stats_failures(days, page, page_size, scenario_name, error_type, keyword)", main)
        ...
        self.assertIn("store.llm_stats_by_scenario(days, only_failures, scenario_name, keyword, run_id, page, page_size)", main)
    ...
    def test_store_defines_expected_llm_aggregation_functions(self):
        ...
        self.assertIn("def llm_stats_failures(days, page, page_size, scenario_name=None, error_type=None, keyword=None):", store)
        self.assertIn("scenario_name=%s", store)
        self.assertIn("error_type=%s", store)
        self.assertIn("agent_role LIKE %s", store)
        self.assertIn("error_msg LIKE %s", store)
        self.assertIn("run_id LIKE %s", store)
        self.assertIn("model LIKE %s", store)
        self.assertIn("def llm_stats_by_scenario(days, only_failures=False, scenario_name=None, keyword=None, run_id=None, page=1, page_size=20):", store)
        self.assertIn("c.run_id=%s", store)
